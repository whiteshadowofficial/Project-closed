        ...
        if (match[1] === '') {
            Asena.commands.map(
                async (command) =>  {
                    ...
                    if (command.desc == '' && !command.usage == '' && command.warn == '') {
                        CMD_HELP += '```┎' + Lang.COMMAND + ':``` ```' + (match.length >= 3 ? (HANDLER + mmatch) : command.pattern) + ':' + '```' + '```' + Lang.EXAMPLE + ':* ```' + command.usage + '```\n\n';
                    }
                    if (!command.desc == '' && command.usage == '' && command.warn == '') {
                        CMD_HELP += '```┝' + Lang.COMMAND + ':``` ```' + (match.length >= 3 ? (HANDLER + mmatch) : command.pattern) + ':' + '```' + '```' + Lang.DESC + ':* ```' + command.desc + '``` \n\n';
                    }
                    if (command.desc == '' && command.usage == '' && !command.warn == '') {
                        CMD_HELP += '```┝' + Lang.COMMAND + ':``` ```' + (match.length >= 3 ? (HANDLER + mmatch) : command.pattern) + ':' + '```' + '```' + Lang.WARN + ':* ```' + command.warn + '```\n\n'
                    }
                    if (!command.desc == '' && !command.usage == '' && command.warn == '') {
                        CMD_HELP += '```┝' + Lang.COMMAND + ':``` ```' + (match.length >= 3 ? (HANDLER + mmatch) : command.pattern) +  ':' + '```' + '```' + Lang.DESC + ':* ```' + command.desc + '``` \n' + '```' + Lang.EXAMPLE + ':* ```' + command.usage + '```\n\n';
                    }
