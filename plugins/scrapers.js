    ...
    Asena.addCommand({pattern: 'song ?(.*)', fromMe: false, desc: Lang.SONG_DESC}, (async (message, match) => { 
        ...
        ffmpeg(stream)
            ...
            .on('end', async () => {
                ...
                reply = await message.client.sendMessage(message.jid,Lang.UPLOADING_SONG,MessageType.text);
                await message.client.sendMessage(message.jid,Buffer.from(writer.arrayBuffer), MessageType.document,{contextInfo: { forwardingScore: 1000, isForwarded: true }, filename: title + '.mp3', mimetype: 'audio/mpeg', quoted: message.data});
            });
    }));
